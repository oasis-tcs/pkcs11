...
representation, the template is considered valid. The template is also
considered valid if the array size is smaller than the number of elements in the
internal representation because the application excluded template attributes to
avoid recursion (see below); otherwise, **CKR_TEMPLATE_INCONSISTENT** **MUST**
be returned.
